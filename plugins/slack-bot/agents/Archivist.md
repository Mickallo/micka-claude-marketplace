...
```bash
"${CLAUDE_PLUGIN_DATA}/venv/bin/python" "${CLAUDE_PLUGIN_ROOT}/src/archivist.py" search "<query>" --top-k 10
```
...
```bash
"${CLAUDE_PLUGIN_DATA}/venv/bin/python" "${CLAUDE_PLUGIN_ROOT}/src/archivist.py" index --topic "<topic>" --source "<source>" --content "<summary>"
```
