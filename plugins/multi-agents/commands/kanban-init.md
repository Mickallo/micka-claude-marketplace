...
Find the plugin's `kanban-board/` directory:

```bash
PLUGIN_BOARD=$(find ~/.claude/plugins/cache -path "*/multi-agents/*/kanban-board" -type d 2>/dev/null | head -1)
```
...
if [ ! -f ~/.claude/kanban-board/package.json ]; then
  cp -R "$PLUGIN_BOARD" ~/.claude/kanban-board
