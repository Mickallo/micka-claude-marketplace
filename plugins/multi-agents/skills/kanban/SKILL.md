...
1. Delegate to `multi-agents:Linear` to fetch the issue:
   ```
   Agent(subagent_type="multi-agents:Linear", prompt="Fetch issue <identifier>. Return: title, description, priority, labels, state. Format as JSON.")
   ```
